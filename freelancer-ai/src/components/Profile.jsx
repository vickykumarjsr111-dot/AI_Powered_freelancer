...
export default function Profile() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      else if (profileData.role === 'client') navigate('/client/dashboard');
      ...
      <div className="profile-card">
        ...
        <div className="role-selector">
          {[
            ...
            { value: 'client',     emoji: '🏢', label: 'Client'     },
          ].map(r => (
            <button
              key={r.value}
              type="button"
              className={`role-btn ${profileData.role === r.value ? 'role-btn--active' : ''}`}
              onClick={() => setProfileData({ ...profileData, role: r.value })}
            >
        ...
        <form className="profile-form" onSubmit={handleSave}>
          ...
          <div className="form-group">
            ...
            <input type="text" name="name" placeholder="Enter your full name"
              value={profileData.name} onChange={handleChange} required />
          </div>

          {/* Show freelancer fields only when role is freelancer */}
          {profileData.role === 'freelancer' && (
            <>
              <div className="form-group">
                <label>Hourly Rate ($)</label>
                <input type="number" name="hourlyRate" placeholder="e.g. 25"
                  value={profileData.hourlyRate} onChange={handleChange} />
              </div>
              ...
              <div className="form-group full-width">
                <label>Bio</label>
                <textarea name="bio" placeholder="Tell clients about yourself..."
              ...
              <div className="form-group">
                ...
                <input type="text" name="portfolioLink"
                  ...
                  value={profileData.portfolioLink} onChange={handleChange} />
              </div>
          ...
          <div className="form-group full-width">
            ...
            <input type="text" name="location" placeholder="India"
              value={profileData.location} onChange={handleChange} />
          </div>

          <div className="profile-actions">
            <button type="button" className="cancel-btn"
              onClick={() => navigate(profileData.role === 'client'
